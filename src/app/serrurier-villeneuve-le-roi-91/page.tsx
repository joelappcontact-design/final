import CityPage from '@/components/CityPage';

const cityData = {
  name: 'Villeneuve-le-Roi',
  department: '91',
  population: '21000',
  priority: 'moyenne',
  zones: ['Centre-ville', 'Quartier de la République', 'Quartier de la Gare', 'Quartier du Parc'],
  description: 'Serrurier d\'urgence Villeneuve-le-Roi 24h/24. Intervention rapide dans tous les quartiers. Artisans vérifiés, service de qualité.',
  keywords: ['serrurier villeneuve-le-roi-91', 'serrurier urgence villeneuve-le-roi-91', 'ouverture porte villeneuve-le-roi-91', 'serrurier 24h villeneuve-le-roi-91', 'dépannage serrure villeneuve-le-roi-91']
};

export default function Page() {
  return <CityPage 
    cityName="Villeneuve-le-Roi"
    citySlug="villeneuve-le-roi-91"
    zones={['Centre-ville', 'Quartier de la République', 'Quartier de la Gare', 'Quartier du Parc']}
    description="Villeneuve-le-Roi - Serrurier d'urgence 24h/24. Intervention rapide dans tous les quartiers. Artisans vérifiés, service de qualité."
    keywords={["serrurier villeneuve-le-roi-91", "serrurier urgence villeneuve-le-roi-91", "ouverture porte villeneuve-le-roi-91", "serrurier 24h villeneuve-le-roi-91", "dépannage serrure villeneuve-le-roi-91"]}
  />;
}

export async function generateMetadata() {
  return {
    title: `Serrurier d'urgence ${cityData.name} 24h/24 - Intervention rapide`,
    description: cityData.description,
    keywords: cityData.keywords.join(', '),
    openGraph: {
      title: `Serrurier d'urgence ${cityData.name} 24h/24`,
      description: cityData.description,
      type: 'website',
      locale: 'fr_FR',
    },
    robots: {
      index: true,
      follow: true,
    },
  };
}